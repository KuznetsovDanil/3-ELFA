...
void Function::out_dot(std::ofstream& out_x, std::ofstream& out_y, bool key)const {
	out_x << x << "\n";
	out_y << y << "\n";
	chart->Series[key ? INTERGAL_DY : APPROCSIMATION]->Points->AddXY(x, y);
}
...
void Function::answer_grafic(const int n, double tay) {
	...
	for (int i = 0; i < n; i++) {
		set_y(answer());
		out_dot(out_x, out_y, INTERGAL_DY);
		x += tay;
	}
	out_x.close();
	out_y.close();
